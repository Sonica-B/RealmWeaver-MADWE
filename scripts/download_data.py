...
class DynamicGameAssetGenerator:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "categories": {
                "textures": {
                    "genres": {
                        "platformer": ["side_scrolling", "metroidvania", "puzzle_platformer", "auto_runner"],
                        "rpg": ["action_rpg", "turn_based", "roguelike", "jrpg", "tactical"],
                        "action": ["beat_em_up", "hack_n_slash", "run_n_gun", "twin_stick"],
                        "puzzle": ["match_3", "physics", "logic", "tile_matching"],
                        "shooter": ["bullet_hell", "side_scrolling", "gallery", "shmup"],
                    ...
                    "styles": {
                        "pixel_art": ["8bit", "16bit", "32bit", "modern_pixel", "micro"],
                        "vector": ["flat", "gradient", "geometric", "minimalist"],
                        "hand_drawn": ["sketch", "ink", "watercolor", "chalk", "pencil"],
                    ...
                    "themes": {
                        ...
                        "historical": ["ancient", "medieval", "renaissance", "industrial"],
                        "abstract": ["geometric", "surreal", "psychedelic", "void"],
                        "steampunk": ["mechanical", "victorian_tech", "brass", "gears"],
                        "post_apocalyptic": ["wasteland", "ruins", "survivor", "mutant"]
                    }
                },
                "sprites": {
                ...
                "gameplay": {
                    "element_types": ["platform", "hazard", "collectible", "interactive"],
            ...
            "asset_structure": {
                "textures": {
                    ...
                    "elements": ["material properties", "surface detail", "color variation", "environmental wear"]
                },
                "sprites": {
                    "pattern": "game-ready sprite asset",
                    "elements": ["clear silhouette", "readable at small size", "animation-friendly design"]
                },
                "gameplay": {
                    "pattern": "functional game element",
                    "elements": ["visual clarity", "gameplay purpose", "feedback indication"]
                }
            }
        }
    ...
    def generate_dynamic_prompt(self, asset_type: str, metadata: Dict[str, str]) -> str:
        ...
        # Base description
        base_pattern = self.asset_structure.get(asset_type, {}).get("pattern", "game asset")
    ...
    def create_asset_image(self, asset_type: str, metadata: Dict[str, Any], 
        ...
        if asset_type == "textures":
            ...
            self._generate_texture_pattern(img_array, base_color, accent_color, highlight_color, metadata)
        elif asset_type == "sprites":
            # Create sprite with clear shape
            self._generate_sprite_pattern(img_array, base_color, accent_color, highlight_color, metadata)
        elif asset_type == "gameplay":
            # Create gameplay element
            self._generate_gameplay_pattern(img_array, base_color, accent_color, highlight_color, metadata)
        else:
            # Default pattern
    ...
    def _generate_texture_pattern(self, img_array: np.ndarray, base_color: Tuple[int, int, int],
                                 accent_color: Tuple[int, int, int], highlight_color: Tuple[int, int, int],
        ...
        if "pixel" in style:
            ...
            for i in range(0, height, block_size):
                for j in range(0, width, block_size):
                    if pattern_choice == 0:
                        ...
                        # Random pattern
                        if random.random() > 0.5:
                            color = np.clip(np.array(base_color) + np.random.randint(-40, 40, 3), 0, 255)
                        else:
                            color = accent_color
                    else:
                        # Gradient blocks
                        progress = (i + j) / (height + width)
                        color = np.array(base_color) * (1 - progress) + np.array(accent_color) * progress
            ...
            if pattern_choice == 0:
                ...
                for _ in range(num_shapes):
                    ...
                    color = np.clip(np.array(color) + np.random.randint(-30, 30, 3), 0, 255)
                ...
                for i in range(0, width if vertical else height, stripe_width):
                    ...
                    color = np.clip(np.array(color) + np.random.randint(-20, 20, 3), 0, 255)
                    if vertical:
            ...
            for _ in range(random.randint(2, 4)):
                ...
                noise = np.random.randn(height // scale, width // scale, 3) * random.randint(15, 35)
            ...
            for i in range(0, height, tile_size):
                for j in range(0, width, tile_size):
                    ...
                    if tile_pattern == 0:
                        ...
                        tile_color = np.clip(
                            np.array(base_color) + np.random.randint(-30, 30, 3), 
                            0, 255
                        )
                        ...
                        img_array[i+center_start:i+center_start+center_size, 
                                 j+center_start:j+center_start+center_size] = accent_color
                    
                    # Always add borders with variation
                    border_width = random.randint(1, 3)
                    border_color = np.clip(np.array(base_color) * random.uniform(0.6, 0.8), 0, 255)
                    img_array[i:i+border_width, j:j+tile_size] = border_color
                    img_array[i+tile_size-border_width:i+tile_size, j:j+tile_size] = border_color
                    img_array[i:i+tile_size, j:j+border_width] = border_color
                    img_array[i:i+tile_size, j+tile_size-border_width:j+tile_size] = border_color
    
    def _generate_sprite_pattern(self, img_array: np.ndarray, base_color: Tuple[int, int, int],
                               accent_color: Tuple[int, int, int], highlight_color: Tuple[int, int, int],
                               metadata: Dict[str, Any]) -> None:
        ...
        if "character" in sprite_type or "hero" in sprite_type or "enemy" in sprite_type:
            ...
            for i in range(body_w // 4):
                shade_color = np.clip(np.array(base_color) * (0.8 + i * 0.2 / (body_w // 4)), 0, 255)
            ...
            head_mask = (x_coords - center_x)**2 + (y_coords - head_center_y)**2 <= head_r**2
            ...
            arm_h = body_h // 2
            # Left arm
            img_array[y1:y1+arm_h, x1-arm_w:x1] = np.clip(np.array(base_color) * 0.85, 0, 255)
            # Right arm
            img_array[y1:y1+arm_h, x2:x2+arm_w] = np.clip(np.array(base_color) * 0.85, 0, 255)
            
            # Legs
            leg_w = body_w // 3
            leg_h = body_h // 2
            # Left leg
            img_array[y2:y2+leg_h, center_x-leg_w-5:center_x-5] = np.clip(np.array(base_color) * 0.8, 0, 255)
            # Right leg
            img_array[y2:y2+leg_h, center_x+5:center_x+leg_w+5] = np.clip(np.array(base_color) * 0.8, 0, 255)
            ...
            if "weapon" in sprite_type:
                ...
                for y in range(blade_y1, blade_y2):
                    ...
                    blade_color = np.array([180, 180, 200]) * (0.7 + 0.3 * center_intensity)
                    img_array[y, x1:x2] = blade_color
                
                # Handle
                handle_x1 = center_x - handle_w // 2
                handle_x2 = center_x + handle_w // 2
                img_array[blade_y2:blade_y2+handle_h, handle_x1:handle_x2] = base_color
                ...
                img_array[blade_y2-guard_h//2:blade_y2+guard_h//2, guard_x1:guard_x2] = np.clip(np.array(base_color) * 0.8, 0, 255)
        ...
        elif "effect" in sprite_type or "explosion" in sprite_type or "magic" in sprite_type:
            ...
            for _ in range(num_particles):
                ...
                particle_size = max(2, int(10 * (1 - distance / (min(width, height) // 3))))
                    ...
                    for dx in range(-particle_size, particle_size + 1):
                        ...
                        if 0 <= px < width and 0 <= py < height:
                            ...
                            if dist <= particle_size:
                                ...
                                # Glowing effect
                                glow_color = np.array(base_color) + np.array([50, 50, 0]) * intensity
                                current_color = img_array[py, px]
                                if not np.array_equal(current_color, [255, 255, 255]):
                                    # Additive blending
                                    img_array[py, px] = np.clip(current_color + glow_color * intensity * 0.5, 0, 255)
                                else:
    ...
    def _generate_gameplay_pattern(self, img_array: np.ndarray, base_color: Tuple[int, int, int],
                                  accent_color: Tuple[int, int, int], highlight_color: Tuple[int, int, int],
                                  metadata: Dict[str, Any]) -> None:
        ...
        element_type = metadata.get("element_types", metadata.get("mechanics", "platform"))
        ...
        if "platform" in str(element_type):
            ...
            if platform_style == 0:
                ...
                for i in range(0, height, brick_height):
                    ...
                    for j in range(-brick_width, width + brick_width, brick_width):
                        ...
                        if x_start < width and x_end > 0:
                            ...
                            img_array[i:min(i+brick_height, height), max(0, x_start):x_end] = brick_color
                            
                            # Mortar lines
                            mortar_color = np.array(base_color) * 0.6
                            if i + brick_height < height:
                                img_array[i+brick_height-2:i+brick_height, max(0, x_start):x_end] = mortar_color
                            if x_end < width:
                                img_array[i:min(i+brick_height, height), x_end-2:x_end] = mortar_color
                ...
                for j in range(0, width, plank_width):
                    plank_color = np.clip(
                        np.array([101, 67, 33]) + np.random.randint(-20, 20, 3),
                        0, 255
                    )
                    ...
                    for i in range(0, height, 3):
                        if random.random() > 0.7:
                            grain_color = np.clip(plank_color * random.uniform(0.8, 1.2), 0, 255)
                            img_array[i:i+1, j:min(j+plank_width, width)] = grain_color
                    
                    # Plank gaps
                    if j + plank_width < width:
                        img_array[:, j+plank_width-2:j+plank_width] = [51, 34, 17]
                ...
                for i in range(0, height, panel_size):
                    for j in range(0, width, panel_size):
                        ...
                        img_array[i:min(i+panel_size, height), j:min(j+panel_size, width)] = panel_color
                        
                        # Rivets at corners
                        rivet_positions = [(5, 5), (5, panel_size-5), 
                                         (panel_size-5, 5), (panel_size-5, panel_size-5)]
                        for ry, rx in rivet_positions:
                            if i+ry < height and j+rx < width:
                                for dy in range(-2, 3):
                                    for dx in range(-2, 3):
                                        if (dx*dx + dy*dy) <= 4:
                                            img_array[i+ry+dy, j+rx+dx] = [60, 60, 80]
            else:
                ...
                for _ in range(100):
                    ...
                    dirt_color = np.clip([92, 64, 51] + np.array([color_var, color_var, color_var]), 0, 255)
            ...
            if edge_style == 0:
                ...
                for i in range(5):
                    ...
                    img_array[-(i+1), :] = img_array[-(i+1), :] * (0.6 + 0.4 * alpha)
            ...
            if hazard_type == 0:
                ...
                for i in range(spike_count):
                    ...
                    spike_height = spike_height_base + random.randint(-height//6, height//6)
                    ...
                    for y in range(spike_height):
                        spike_half_width = max(1, (spike_height - y) * spike_width // (2 * spike_height))
                        
                        for x in range(max(0, spike_center - spike_half_width), 
                                     min(width, spike_center + spike_half_width)):
                            # Distance from center for shading
                            dist_from_center = abs(x - spike_center) / max(1, spike_half_width)
                            intensity = 1.0 - (y / spike_height) * 0.5 - dist_from_center * 0.3
                        ...
                        for px in range(max(0, x-radius), min(width, x+radius+1)):
                            ...
                            if dist <= radius:
                                ...
                                img_array[py, px] = np.clip(
                                    current * 0.3 + np.array(lava_color) * 0.7,
                                    0, 255
                                )
                ...
                for _ in range(random.randint(30, 50)):
                    ...
                    bubble_color = np.array([80, 200, 80]) + np.random.randint(-40, 40, 3)
                    img_array[mask] = np.clip(bubble_color, 0, 255)
                    
                    # Bubble shine
                    shine_mask = (x_coords - (x-radius//3))**2 + (y_coords - (y-radius//3))**2 <= (radius//3)**2
                ...
                for i in range(blade_count):
                    ...
                    # Center hub
                    y_coords, x_coords = np.ogrid[:height, :width]
                    hub_mask = (x_coords - blade_x)**2 + (y_coords - blade_y)**2 <= (blade_radius//4)**2
            ...
            if collectible_type == 0:
                ...
                ring_mask = (distances <= coin_radius) & (distances >= coin_radius * 0.8)
                ...
                for y in range(height):
                    for x in range(width):
                        if inner_mask[y, x]:
                            dist_from_top_left = np.sqrt((x - center_x + coin_radius//3)**2 + 
                                                       (y - center_y + coin_radius//3)**2)
                            if dist_from_top_left < coin_radius // 2:
                                shine_factor = 1 - dist_from_top_left / (coin_radius // 2)
                                current = img_array[y, x]
                                img_array[y, x] = np.clip(current + shine_factor * 50, 0, 255)
                ...
                for y in range(height):
                    for x in range(width):
                        ...
                        if (nx**2 + ny**2 - 1)**3 - nx**2 * ny**3 <= 0:
                            ...
                            heart_color = (255 * intensity, 100 * intensity, 100 * intensity)
            ...
            if interactive_type == 0:
                ...
                # Draw button
                button_area = img_array[button_margin:-button_margin, button_margin:-button_margin]
                ...
                for i in range(depth):
                    ...
                    button_area[i, i:] = np.clip(np.array(button_color) * (1.3 - fade * 0.3), 0, 255)
                    button_area[i:, i] = np.clip(np.array(button_color) * (1.2 - fade * 0.2), 0, 255)
                    # Bottom and right shadows
                    button_area[-i-1, i:-i] = np.clip(np.array(button_color) * (0.7 + fade * 0.3), 0, 255)
                    button_area[i:-i, -i-1] = np.clip(np.array(button_color) * (0.6 + fade * 0.4), 0, 255)
                ...
                base_x = width // 2
                base_y = height // 2
                img_array[base_y-base_size//2:base_y+base_size//2,
                         base_x-base_size//2:base_x+base_size//2] = (40, 30, 15)
                ...
                # Door style
                door_margin = 20
                door_area = img_array[door_margin:-door_margin, door_margin:-door_margin]
                ...
                for radius in range(max_radius, 0, -5):
                    y_coords, x_coords = np.ogrid[:door_area.shape[0], :door_area.shape[1]]
            ...
            if generic_type == 0:
                ...
                flag_color = random.choice([(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0)])
                ...
                # Rails
                rail_height = 10
                img_array[height//2-rail_height:height//2+rail_height, :] = (40, 40, 40)
                
                # Track details
                for x in range(0, width, 30):
                    img_array[height//2-rail_height:height//2+rail_height, x:x+5] = (80, 80, 80)
                ...
                for i in range(coil_count):
                    ...
                    for x in range(width//4, 3*width//4):
                        ...
                        if 0 <= coil_y < height:
                            ...
                            img_array[coil_y-2:coil_y+2, x] = np.array([180, 180, 200]) * intensity
                ...
                for y in range(q_height):
                    for x in range(q_width):
                        # Simple ? shape
                        if (y < q_height//3 and abs(x - q_width//2) < q_width//3) or \
                           (q_height//2 < y < 2*q_height//3 and x > q_width//2):
                            box_area[q_center_y - q_height//2 + y, 
                                    q_center_x - q_width//2 + x] = highlight_color
                        elif 3*q_height//4 < y and abs(x - q_width//2) < q_width//6:
                            box_area[q_center_y - q_height//2 + y, 
                                    q_center_x - q_width//2 + x] = highlight_color
    
    def generate_comprehensive_assets(self, output_dir: Path | str, 
                                    samples_per_category: int = 10) -> None:
        ...
        for asset_type, type_categories in self.categories.items():
            ...
            with tqdm(total=len(combinations) * samples_per_category, 
                     desc=f"{asset_type}") as pbar:
    ...
    def _generate_combinations(self, categories: Dict[str, List[str]]) -> List[Dict[str, str]]:
            ...
            for primary_val in categories[primary_cat]:
                if secondary_cat:
                    for secondary_val in categories[secondary_cat]:
                        combinations.append({
                            primary_cat: primary_val,
                            secondary_cat: secondary_val
                        })
                else:
        ...
        for asset_type in generator.categories:
            for cat in generator.categories[asset_type]:
                if isinstance(generator.categories[asset_type][cat], list):
                    generator.categories[asset_type][cat] = generator.categories[asset_type][cat][:2]
